...
async def create_session(body: SessionCreate):
    ...
    session = db_service.create_session(
        sid, title=body.title, model=body.model, language=body.language
    )
    return session
...
async def bulk_rename_sessions(body: BulkSessionRenameRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Bulk rename failed: {e!s}")
...
async def clone_session(session_id: str):
    ...
    for msg in messages:
        db_service.save_message(new_id, msg["role"], msg["content"], msg["sources"])
...
async def update_session(session_id: str, body: SessionUpdate):
    db_service.update_session(
        session_id, title=body.title, model=body.model, language=body.language
    )
