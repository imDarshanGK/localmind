        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          ...
          {filtered.map(s => (
            <div key={s.id} className={`group flex items-center gap-1 rounded-lg mb-0.5 transition
              ...
              <button onClick={() => { onLoadSession(s.id); setIsOpen(false); }}
              ...
              <button onClick={()=>onDeleteSession(s.id)}
                className="opacity-100 md:opacity-0 group-hover:opacity-100 text-gray-600 hover:text-red-400 px-2 py-2 transition text-xs">
                ×
              </button>
            </div>
          ))}
