...
export default function Sidebar({
  ...
  // Track mobile menu visibility status
  const [isOpen, setIsOpen] = useState(false);

  const modelList = models.length > 0 ? models.map((m) => m.name) : ["llama3", "mistral", "phi3", "gemma2"];
  const filtered = sessions.filter((s) => s.title?.toLowerCase().includes(search.toLowerCase()));

  // Wrap button click triggers to auto-close the drawer panel on mobile viewports
  ...
  return (
    <>
      {/* 1. Mobile Hamburger Toggle Floating Button */}
      <button
      ...
      {/* 2. Mobile Dim Dimming Backdrop Screen Overlay */}
      {isOpen && (
        <div
      ...
      {/* 3. Responsive Sliding Side Drawer Layout */}
      <div
        className={`
          fixed inset-y-0 left-0 z-40 transform transition-transform duration-300 ease-in-out
          md:relative md:transform-none md:translate-x-0 md:z-auto
          w-64 flex flex-col bg-gray-900 border-r border-gray-800 shrink-0
      ...
      >
        ...
        {/* Filtered Active Session Row Wrapper */}
        <div className="flex-1 overflow-y-auto px-2 py-2" data-testid="sidebar-sessions-list">
          ...
          {filtered.map((s) => (
            ...
            >
              <button
                onClick={() => handleSessionSelection(s.id)}
              ...
              >
                ...
                {s.message_count > 0 && <span className="ml-1 text-gray-600">{s.message_count}</span>}
              </button>
              <button
