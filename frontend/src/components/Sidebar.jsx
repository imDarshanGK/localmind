...
export default function Sidebar({ sessions, currentSession, onNewChat, onLoadSession, onDeleteSession, model, models, onModelChange, language, onLanguageChange }) {
  ...
  return (
    <aside 
      ...
      className="w-64 flex flex-col bg-gray-900 border-r border-gray-800 shrink-0"
    >
      ...
      {/* Chat Sessions Navigation Landmark List */}
      <nav 
        ...
        {filtered.map(s => (
          <div key={s.id} className={`group flex items-center gap-1 rounded-lg mb-0.5 transition
            ...
            <button 
              ...
              aria-current={currentSession === s.id ? "true" : undefined}
              className="flex-1 text-left text-xs px-3 py-2 truncate text-gray-400 group-hover:text-gray-200"
            >
              <span className={currentSession === s.id ? "text-white font-medium" : ""}>
              ...
              {s.message_count > 0 && (
                <span className="ml-1 text-gray-600" aria-label={`${s.message_count} messages`}>
                  ({s.message_count})
                </span>
              )}
            </button>
